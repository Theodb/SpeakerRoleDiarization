...
def process_option1(stereo_path, client_audio_path, agent_audio_path):
    """
    Option 1: Use ASR (Whisper) directly on both channels.
    """
    try:
        ...
        df.sort_values(by=['start', 'duration'], inplace=True)
        df['text'] = df['text'].fillna('')

        output_dir = "/Users/research-team/theo_db/data/new_data/whisper_ts"
        os.makedirs(output_dir, exist_ok=True)
        out_csv = os.path.join(output_dir, os.path.basename(stereo_path).replace('.wav', '') + '.csv')
        df.to_csv(out_csv, index=False)
    except Exception as e:
        logging.error(f"Error processing option 1 for file {stereo_path}: {e}")
